...
export default function SocialLinkAlt({
  ...
  return (
    <a
      className={`relative rounded-2xl overflow-hidden ${subwindow ? "bg-window-in-window-outline-gradient" : "bg-border-gradient"} p-px h-fit group hover:rotate-2 transition-transform ${classname}`}
      href={href}
      target="_blank"
    >
      <div
      ...
      >
        <div className="absolute h-[220%] w-0 group-hover:w-[220%] rotate-20 translate-[-50%] inset-0 transition-all duration-500 pointer-events-none z-0 bg-linear-to-br from-violet-400 to-deep-blue"></div>
